...
性能监控需要覆盖四个维度。**吞吐量**方面，重点关注 QPS（每秒查询数）和 TPS（每秒事务数），通过 `SHOW GLOBAL STATUS LIKE 'Queries%'` 获取，健康状态下数值应平稳，无明显骤降。**响应时间**方面，以 P95/P99 延迟和平均查询时间为核心指标，结合慢查询日志和应用端 APM 工具监控，目标是 P95 < 100ms。**资源利用率**方面，通过操作系统工具（如 top、iostat）监控 CPU、内存和磁盘 IOPS，保持 CPU 使用率 < 70%，避免出现 Swap。**数据库核心指标**方面，重点是 Buffer Pool 命中率（目标 >99%）、锁等待次数和当前连接数，通过 `SHOW ENGINE INNODB STATUS` 和 Performance Schema 实时掌握。
**核心监控工具**
...
---
**知识库索引**
- [[../../../21_MySqlKnowledge/05_架构与运维/02、性能优化与监控|📖 性能优化与监控]]
