...
- 高吞吐量，适合大数据处理场景
- 持久化能力强，消息不会丢失
**技术选型**：RabbitMQ适合中小规模微服务、复杂路由场景，轻量级、易部署；Kafka适合大规模系统、高吞吐量，高并发、持久化、适合日志处理。
**配置示例**（RabbitMQ）：
```yaml
spring:
  rabbitmq:
...
```
**源码机制**：`RefreshEventListener`监听`RefreshRemoteApplicationEvent`事件；收到事件后，通过`ContextRefresher`刷新配置Bean；`@RefreshScope`标注的Bean会被重新创建，注入新配置。
...
核心区别在于**编程模型和耦合度**的差异。传统消息队列方式需要直接使用MQ客户端API，代码与特定MQ强耦合，配置复杂；Spring Cloud Stream通过注解驱动的声明式编程模型，通过Binder抽象屏蔽了底层中间件的差异，切换消息中间件只需修改配置，不需要修改代码。
**其他优势**：Spring Cloud Stream提供测试绑定器（TestBinder），便于编写单元测试；与Spring生态深度集成，自动处理序列化等。
...
**主要功能**：连接管理、目的地抽象（将中间件的特定目的地概念抽象为统一的Binding）、消息转换（处理序列化/反序列化）、错误处理。
**内置Binder实现**：
