...
###### 15. Sentinel 的限流算法原理是什么？漏桶和令牌桶有什么区别？——高频面试引导问题

**三种限流控制器**：

| 控制器 | 算法 | 类名 | 特点 |
|--------|------|------|------|
| 直接拒绝 | 滑动窗口计数 | `DefaultController` | 超过 QPS 直接拒绝 |
| 匀速排队 | 漏桶 | `RateLimiterController` | 请求排队等待，平滑流量 |
| 预热/冷启动 | 令牌桶（改进） | `WarmUpController` | 慢慢升温，防冷启动雪崩 |

**漏桶（匀速排队）源码核心逻辑**：

```java
// RateLimiterController：记录上次请求时间，计算下次允许时间
public boolean canPass(Node node, int acquireCount, boolean prioritized) {
    long currentTime = TimeUtil.currentTimeMillis();
    // 根据QPS限制计算两次请求的最小间隔时间（单位ms）
...
```java
// WarmUpController 核心参数示例
// count = 3, coldFactor = 3（默认）, warmUpPeriod = 4s
// warningToken = (4 * 3) / (3 - 1) = 6
// maxToken = 6 + (2 * 4 * 3) / (1 + 3) = 12
// slope ≈ 0.111

// 当桶内剩余令牌数 > warningToken 时，说明系统冷，限制 QPS
// 随着令牌被消耗，QPS 逐渐升到 count 的值（预热完成）
```

**回答示例**：

> Sentinel 有三种流量控制器：
> 1. **默认（直接拒绝）**：超过 QPS 阈值直接返回错误，适合普通限流
> 2. **匀速排队（漏桶）**：请求以固定间隔通过，超过等待时间才拒绝，适合流量削峰
> 3. **预热冷启动（令牌桶）**：系统刚启动时 QPS 较低，随时间线性提升到配置值，防止冷启动时大流量打垮刚起的服务
>
> 追问：漏桶和令牌桶的区别？
> - 漏桶：出口速率固定，输入流量平滑化输出，适合对流量精确整形
> - 令牌桶：可以应对短时突发（桶里攒了令牌），更灵活
> - Sentinel 的漏桶（匀速排队）会让请求等待，令牌桶（预热）则是控制速率升降

---

###### 16. Sentinel 的 Slot 链原理是什么？——高频面试引导问题

**ProcessorSlotChain 执行链**：

```
NodeSelectorSlot     ← 构建节点树（资源-调用链关系）
ClusterBuilderSlot   ← 统计集群数据
StatisticSlot        ← 统计 QPS、RT、线程数（核心统计）
ParamFlowSlot        ← 热点参数限流
SystemSlot           ← 系统保护（CPU/RT/QPS自适应）
AuthoritySlot        ← 黑白名单
FlowSlot             ← 限流规则检查
DegradeSlot          ← 熔断降级
```
...
> Sentinel 基于责任链模式，每次请求进入资源时都会经过完整的 Slot 链：
> - `StatisticSlot` 负责收集 QPS、RT、线程数等指标（滑动时间窗口统计）
> - `FlowSlot` 根据配置的限流规则，选择对应的 Controller（漏桶/令牌桶/计数）来决定是否放行
> - `DegradeSlot` 根据降级规则（慢调用比例、异常比例、异常数）决定是否触发熔断
>
> 每个 Slot 既收集数据又可以做决策，通过链式调用形成完整的流量治理体系。
