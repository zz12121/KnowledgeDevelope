...
- **Nginx**：C语言实现的高性能**流量网关**，专注于负载均衡、反向代理、静态内容缓存。适合作为入口层的流量分发和负载均衡器。
- **Spring Cloud Gateway**：Spring官方出品，基于**WebFlux响应式编程模型**（非阻塞IO），与Spring Cloud生态无缝集成，性能优异。适合Java技术栈的微服务项目，特别是Spring Cloud体系。
- **Netflix Zuul**：早期微服务网关经典方案，Zuul 2.x支持异步非阻塞模型。与Netflix OSS组件集成度高，适合传统Spring Cloud项目。
- **Kong**：基于Nginx，**插件生态丰富**，API管理能力强，社区成熟。适合需要丰富插件支持、多语言异构架构的大规模分布式系统。
- **Apache APISIX**：动态、实时、高性能的API网关，支持热加载和多种协议。适合云原生环境，对性能和控制力要求极高的场景。
...
📖 [[../../../../27_SpringCloudKnowledge/04_熔断降级与网关/01、熔断降级与服务网关#七、Zuul]]
...
###### 8. Zuul 的过滤器类型有哪些？
Zuul的过滤器主要有四种类型：
- **`pre`**：路由之前执行，负责认证、安全、路由决策、日志
- **`route`**：路由过程中执行，负责请求转发（如HTTP/Ribbon）
- **`post`**：路由之后（收到响应）执行，负责响应头处理、指标收集、流式响应
- **`error`**：发生错误时执行，负责错误处理、统一异常响应
...
核心区别是**编程模型和性能**：Gateway基于响应式WebFlux，非阻塞IO，高并发下资源消耗少；Zuul 1.x是阻塞IO的Servlet模型，每请求一个线程，高并发下有性能瓶颈。
另外Gateway是Spring"亲生子"，与Spring Cloud生态无缝集成；Zuul是Netflix OSS，相关组件已逐步进入维护模式。对于新项目，**Spring Cloud Gateway是更推荐的选择**。

📖 [[../../../../27_SpringCloudKnowledge/04_熔断降级与网关/01、熔断降级与服务网关#六、Spring Cloud Gateway]]

###### 11. Gateway 的核心概念有哪些（路由、断言、过滤器）？
Spring Cloud Gateway的架构基于三个核心概念：
1. **路由（Route）**：网关的基本构建块，包含ID（唯一标识）、目标URI（转发地址）、断言集合（匹配条件）、过滤器集合（请求/响应处理逻辑）。
2. **断言（Predicate）**：Java 8的`Predicate`接口实现，用于检查HTTP请求的各项属性（Header、Method、Path、Query参数等），判断当前请求是否与该路由匹配。例如，`Path=/api/users/**`就是一个路径断言。
3. **过滤器（Filter）**：`GatewayFilter`接口的实例，可以在将请求发送到下游之前（"pre"）或之后（"post"）拦截并修改请求和响应。用于实现横切关注点，如添加请求头、权限校验、限流等。
...
###### 13. Gateway 的断言类型有哪些？
Spring Cloud Gateway提供了丰富的内置断言工厂：
**时间相关断言**
```yaml
predicates:
...
predicates:
  ...
  - Host=**.example.com
```
**高级断言**：`WeightRoutePredicateFactory`支持权重路由，用于灰度发布。
**组合断言**：支持AND逻辑组合：
```yaml
predicates:
  - Path=/api/**
...
Gateway过滤器分为**全局过滤器**和**网关过滤器**两大类。
**网关过滤器（GatewayFilter）**：作用于特定路由，通过配置在routes下实现：
- 请求/响应修改类：`AddRequestHeaderFilter`、`RewritePathFilter`、`PrefixPathFilter`等
- 参数处理类：`AddRequestParameterFilter`、`RewriteRequestParameterFilter`等
- 特殊功能类：`StripPrefixFilter`、`RetryFilter`、`RequestRateLimiterFilter`等
**全局过滤器（GlobalFilter）**：全局生效，实现`GlobalFilter`接口：
...
核心区别就是**作用范围**：全局过滤器对所有路由生效，实现`GlobalFilter`接口，由Gateway框架自动加载；局部过滤器只对配置了它的特定路由生效，实现`GatewayFilter`接口，需在routes配置中显式指定。
功能定位上，全局过滤器承载核心基础设施功能（负载均衡、路由转发等），局部过滤器承载业务相关功能（头信息处理、路径修改等）。
...
```
**生产实践**：结合Nacos配置中心，实现路由规则的热更新，支持灰度发布、AB测试等场景。
...
```
**JVM优化**：合理分配堆内存和直接内存，使用G1垃圾收集器，减少GC停顿。
**路由配置优化**：
- **减少过滤器链长度**：移除不必要的过滤器
- **合理使用缓存**：对静态资源启用缓存
- **异步处理**：避免阻塞操作，使用响应式编程
...
###### 25. API 网关的安全设计有哪些方面？
**多层次安全防护**
1. **网络层安全**：TLS/SSL终端加密、IP白名单限制、DDoS防护
2. **身份认证**：OAuth2.0/OpenID Connect、JWT令牌验证、API密钥认证
3. **授权控制**：RBAC基于角色的访问控制、ABAC基于属性的访问控制、细粒度权限管理
4. **安全审计**：完整请求日志记录、异常行为监控告警、安全事件追踪
**具体安全配置**
```yaml
spring:
  cloud:
