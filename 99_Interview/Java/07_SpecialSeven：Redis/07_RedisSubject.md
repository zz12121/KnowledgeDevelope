...
|     | Redis 的链表数据结构的特征有哪些？                                        |     |     |
|     | Ziplist和Quicklist这两个压缩列表有啥特点？                               |     |     |
|     | Zset为啥用跳表不用红黑树？                                             |     |     |
|     | 你说说 Redis 中的跳表                                              |     |     |
|     | ListPack是什么？和Ziplist有啥不同？                                   |     |     |
|     | Redis 的应用场景有哪些？                                             |     |     |
|     | 说一下 Redis 通讯协议是什么？有什么特点？                                    |     |     |
|     | 有了解过 Redis 事务吗？原理是什么？                                       |     |     |
|     | Redis事务和MySQL事务有啥本质区别？                                      |     |     |
|     | Redis 事务有相关命令有哪些                                            |     |     |
|     | Redis 事务的注意点有哪些？                                            |     |     |
|     | Redis 保证原子性吗支持回滚吗？支与不支持的理由是什么？                              |     |     |
|     | Redis 事务支持隔离性吗                                              |     |     |
|     | Redis中的Lua 脚本是什么？如何使用？                                      |     |     |
...
|     | 哨兵选举过程是怎么样的？                                                |     |     |
|     | Cluster模式和Sentinel模式怎么选？                                    |     |     |
|     | Redis 的脑裂问题是怎么解决的？                                          |     |     |
|     | Redis 集群最大节点个数是多少？                                          |     |     |
|     | Redis 集群的主从复制模型是怎样的？                                        |     |     |
|     | 说一下 Redis 的主从复制模式的优缺点?                                      |     |     |
|     | Redis 主从复制延迟的常见原因有哪些？                                       |     |     |
|     | Redis 主从复制的常见拓扑结构有哪些？                                       |     |     |
|     | 新的主节点是怎样被挑选出来的？                                             |     |     |
|     | 说说你知道哪些 Redis 分片技术？                                         |     |     |
|     | Redis 集群中数据如何分区？                                            |     |     |
|     | Redis 集群中，如何根据键定位到对应的节点？                                    |     |     |
|     | 说说 Redis 哈希槽的概念？                                            |     |     |
...
|     | 你了解Redis 的订阅发布功能吗？                                          |     |     |
|     | Redis Geo地理定位功能解是怎么实现的？                                     |     |     |
|     | 如何解决 Redis 中的热点 key 问题？                                     |     |     |
|     | Redis的cpu飙升如何排查？                                            |     |     |
|     | Redis 中的内存碎片化是什么？如何进行优化？                                    |     |     |
|     | 如何利用 Redis 实现排行榜功能？                                         |     |     |
|     | 如何利用 Redis 如何统计千万用户的访问量？                                    |     |     |
|     | 如何在 Redis 中实现队列和栈数据结构？                                      |     |     |
|     | Redis源码里有哪些精妙设计？举几个例子                                       |     |     |
