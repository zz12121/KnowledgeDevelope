...
> 旋转是红黑树保持平衡的核心操作，不改变二叉搜索树的中序遍历顺序。

**左旋（rotateLeft）**：将某个节点 x 向左旋转，变成其右子节点的左子节点
...
**右旋（rotateRight）**：将某个节点 y 向右旋转，变成其左子节点的右子节点
...
### 补充二：HashMap 中红黑树的平衡调整

> HashMap 中的 TreeNode 在插入后会自动进行平衡调整，主要包含两个操作：**变色**和**旋转**。

**变色规则**：
- 红色节点的子节点必须是黑色
- 从根到叶子的路径上黑色节点数量必须相同

**旋转触发条件**（在 HashMap put 过程中）：
1. 当前节点的父节点是红色，且祖父节点的另一个子节点也是红色 → 变色
2. 当前节点是右子节点，且父节点是红色 → 左旋
3. 当前节点是左子节点，且父节点是红色 → 右旋

### 补充三：从 2-3 树理解红黑树

> 理解红黑树的最好方式是先理解 2-3 树，再理解二者的等价转换。

**2-3 树的特点**：
- 2-节点：保存 1 个值，最多 2 个子节点
- 3-节点：保存 2 个值，最多 3 个子节点
- 绝对平衡：所有叶子节点在同一层

**2-3 树转换为红黑树**：
- 2-节点 → 黑色节点
- 3-节点 → 黑色节点 + 红色左子节点（红色表示它是融合的）
...
2. 根节点是黑色
3. 每个叶子节点（NIL）是黑色
4. 如果一个节点是红色，则它的子节点必须是黑色（不能有连续的红色）
