...
## 五、PDF补充：association 与 collection 详解

### association（has-one 一对一）

用于解决"has one"类型的关系，表示一个对象至多有一个关联对象。

**子查询方式**：
```xml
<!-- MessageDetailMapper.xml -->
<select id="getMessageByMsgId" parameterType="string" resultType="messageDetail">
    select id, msg_id, detail_content, create_time, update_time
...
### collection（has-many 一对多）

用于解决"has many"类型的关系，表示一个对象有多个关联对象。

**子查询方式**：
```xml
<!-- UserContactMapper.xml -->
<select id="getUserContactByUserId" parameterType="long" resultType="vo.UserContact">
    select id, user_id, contact_type, contact_value, create_time, update_time
    from tb_user_contact
...
| 方式 | 优点 | 缺点 | 适用场景 |
|------|------|------|---------|
| 嵌套查询（N+1问题）| SQL 简单，延迟加载 | 多次查询，性能差 | 数据量小 |
| 嵌套结果（推荐）| 一次查询全部 | SQL 复杂 | 数据量大 |
